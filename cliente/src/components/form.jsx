                        ...
                        {postData.sections.map((section, index) => (
                            <div key={index} className='create-section'>
                                <div className='inner-section'>
                                    <div><button type='button' onClick={() => handleAddContent(index)} className='content-button'>Agregar contenido</button><input type='number' name='section-order' value={index} onChange={(e) => handleSectionOrderChange(index,e)} min='0' className='order-input'/></div>
                                ...
                                {index > 0 ? <button type='button' onClick={() => handleDeleteSection(index)} className='content-button'>Eliminar Seccion</button> : <></>}
                                {section.contents.map((content, contentIndex) => (
                                    <div key={contentIndex} className='create-content'>
                                        ...
                                        <input type='number' name='content-order' value={contentIndex} onChange={(e) => {handleContentOrderChange(index,contentIndex,e)}} min='0' className='order-input'></input>
                                        </div>
                                        {
                                            content.type === 'paragraph' ?
                                            <>
                                            <textarea type='text' name='data' value={content.data} placeholder='Contenido' onChange={(e) => handleContentChange(index, contentIndex, e)} className='input-text-2'/>
                                            </> : content.type === 'list' 
                                            ? <>
                                            <button type="button" onClick={() => handleAddListItem(index, contentIndex)}>Agregar elemento</button>
                                                {content.data.map((item, itemIndex) => (
                                                    <div key={itemIndex} className='inner-content-list'>
                                                        <textarea type='text' value={item} onChange={(e) => handleListItemChange(index, contentIndex, itemIndex, e)} placeholder={`Elemento ${itemIndex + 1}`} className='input-text-2'/>
                                                        <input type='number' name='content-order' value={itemIndex} onChange={(e) => handleListOrderChange(index,contentIndex,itemIndex,e)} className='order-input' min='0'></input>
